...
local widget_directory = package_path:match('(.*/)')
local widget_icon_dir = widget_directory .. 'icons/'

local user_icon_dir = '/var/lib/AccountsService/icons/'

local title_table = {
  'Hey, I have a message for you', 'Listen here you little shit!',
  'Le\' me tell you a secret', 'I never lie', 'Message received from your boss'
}
...
awful.spawn.easy_async_with_shell(
  user_jpg_checker, function(already)
            ...
            check_profile_pic_cmd, function(status)
              ...
              if status:match('detected') then
                ...
                gears.timer.start_new(
                  1, function()
                    profile_imagebox.icon:set_image(
                      widget_icon_dir .. 'user.jpg')
                  end)
...
awful.spawn.easy_async_with_shell(
  get_distro_name_cmd, function(out)
...
awful.spawn.easy_async_with_shell(
  'uname -r', function(out)

    local kname = out:gsub('%\n', '')
